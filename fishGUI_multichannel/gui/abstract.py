...
class abstract():
    ...
    def _load_cytoplasms(self, cyto_paths: list[pathlib.Path]) -> tuple[np.ndarray, np.ndarray]:
        ...
        for cyto_path in cyto_paths:
            ...
            if "647" in stem:
                img_647 = clahe(zprojected, clip_limit=2.0, tile_size=(8,8))
            elif "488" in stem:
                ...
                img_555 = clahe(zprojected, clip_limit=4.0, tile_size=(8,8))
            elif "594" in stem:
                img_594 = clahe(zprojected, clip_limit=4.0, tile_size=(8,8))
            else:
