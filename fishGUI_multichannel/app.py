...
class FishGUI(object):
    def __init__(self, root, export_pairing_debug_pdf: bool = False):
        ...
        self.__export_pairing_debug_pdf = export_pairing_debug_pdf
    ...
    def _get_mode_banner_style(self, mode: str) -> tuple[str, str, str]:
        ...
        styles = {
            ...
            "cytoplasm": ("Cytoplasm Mode", "#D97706", "white"),
        }
    ...
    def shouldExportPairingDebugPdf(self) -> bool:
        """Return True when export should write the optional pairing debug PDF."""
        return self.__export_pairing_debug_pdf


def parse_cli_args():
    """Read command-line options for launching the GUI."""
    parser = argparse.ArgumentParser(description="Start the multichannel FISH GUI")
    parser.add_argument(
        "--export-pairing-debug-pdf",
        action="store_true",
        help="Write the optional pairing debug PDF during export",
    )
...
def main():
    ...
    args = parse_cli_args()
    root = tk.Tk()
    app = FishGUI(root, export_pairing_debug_pdf=args.export_pairing_debug_pdf)
    root.mainloop()
