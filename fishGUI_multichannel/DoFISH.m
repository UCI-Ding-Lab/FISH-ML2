...
%             [butLS+3.3*butSz,0.32,butSz,butSz],'Callback',{@iPadKeyPress,'t'})
        ...
        function DeleteSegment % button "delete"
            if strcmp(State,'delete')
                disp('--- DoFISH delete debug ---')
                disp('Delete mode turned OFF')
                disp('--- end delete debug ---')
                ...
                disp('--- DoFISH delete debug ---')
                disp('Delete mode turned ON')
                disp(['current frame = ', num2str(frame)])
                disp(['current cell count = ', num2str(length(Tracked{frame}.cells))])
                disp('Click inside a cell to test delete')
                disp('--- end delete debug ---')
            ...
            function btndown(src,evnt)
                disp('--- DoFISH delete debug ---')
                disp('Mouse click received in delete mode')
                ...
                disp(['raw click point x/y = ', mat2str(p(1,1:2))])
                disp(['converted click coord row/col = ', mat2str(coord)])
                disp(['cell count before = ', num2str(length(Tracked{frame}.cells))])
                ...
                disp(['allpos size = ', mat2str(size(allpos))])
                disp(['allsize size = ', mat2str(size(allsize))])
        
                for debug_cell=1:min(10,length(Tracked{frame}.cells))
                    debug_pos = Tracked{frame}.cells{debug_cell}.pos;
                    debug_size = Tracked{frame}.cells{debug_cell}.size;
                    debug_max = double(debug_pos) + double(debug_size);
        
                    disp(['cell ', num2str(debug_cell), ...
                          ' pos = ', mat2str(debug_pos), ...
                          ' size = ', mat2str(debug_size), ...
                          ' max = ', mat2str(debug_max)])
                end
        
                isincell=all(bsxfun(@minus,allsize+allpos,coord)>0 & bsxfun(@minus,allpos,coord)<0,2);
                clicked=find(isincell,1);
        
                disp(['number of containing boxes = ', num2str(sum(isincell))])
                disp(['clicked = ', mat2str(clicked)])
        
                if isempty(clicked)
                    disp('No cell was deleted because the click was outside all pos/size boxes')
                    disp('--- end delete debug ---')
                    return
                end
        
                debug_cell_data = Tracked{frame}.cells{clicked};
        
                disp(['clicked cell index = ', num2str(clicked)])
                disp(['clicked cell pos = ', mat2str(debug_cell_data.pos)])
                disp(['clicked cell size = ', mat2str(debug_cell_data.size)])
                disp(['clicked cell mask size = ', mat2str(size(debug_cell_data.mask))])
                disp(['clicked cell mask sum = ', num2str(sum(debug_cell_data.mask(:)))])
                disp(['clicked cell progenitor class = ', class(debug_cell_data.progenitor)])
                disp(['clicked cell descendants class = ', class(debug_cell_data.descendants)])
        
                if isfield(debug_cell_data, 'nucleus_mask')
                    disp(['clicked cell nucleus_mask size = ', mat2str(size(debug_cell_data.nucleus_mask))])
                    disp(['clicked cell nucleus_mask sum = ', num2str(sum(debug_cell_data.nucleus_mask(:)))])
                else
                    disp('clicked cell has no nucleus_mask field')
                end
                ...
                if frame>1
                    disp('Renumbering descendants in previous frame')
                ...
                if frame<length(Tracked)
                    disp('Renumbering progenitors in next frame')
                    for ccell=1:length(Tracked{frame+1}.cells)
                ...
                cell_count_before_delete = length(Tracked{frame}.cells);
        
                Tracked{frame}.cells(clicked)=[];
                Tracked{frame}.Locked=0;
        
                for num = 1:channel_num
                    disp(['Deleting FISH cell for channel ', num2str(num)])
                    if length(FISH{num, frame}.cells) >= clicked
                        FISH{num, frame}.cells(clicked)=[];
                    else
                        disp(['FISH channel ', num2str(num), ' has fewer cells than Tracked'])
                    end
                end
        
                disp(['cell count before delete = ', num2str(cell_count_before_delete)])
                disp(['cell count after delete = ', num2str(length(Tracked{frame}.cells))])
                disp('Calling plotframe after delete')
                disp('--- end delete debug ---')
