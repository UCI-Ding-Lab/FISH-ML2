...
def test_initialization_exposes_available_channels_and_original_image_paths(dummy_image_paths):
    ...
    nucleus, cyto_paths, cyto_channels, channels = dummy_image_paths
    gui = MagicMock()
    gallery_frame = MagicMock()

    with patch("fishGUI_multichannel.gui.abstract.tifffile.imread", return_value=np.zeros((10, 10))), \
         ...
         patch("fishGUI_multichannel.gui.abstract.normalize_to_uint8", side_effect=lambda x: x), \
         patch("fishGUI_multichannel.gui.abstract.remove_outliers", side_effect=lambda x, **_: x), \
         patch("fishGUI_multichannel.gui.abstract.clahe", side_effect=lambda x, **_: x), \
    ...
    assert {"647", "488"}.issubset(abs_obj.getImgNumpyCyto().keys())
