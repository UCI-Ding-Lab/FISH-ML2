...
class Fish():
    ...
    def setup__asset(self):
        ...
        self.model_folder_path = pathlib.Path(self.config["general"]["model_folder_path"])
        if not self.asset_folder_path.is_absolute():
            self.asset_folder_path = (self.config_path.parent / self.asset_folder_path).resolve()
        if not self.model_folder_path.is_absolute():
            self.model_folder_path = (self.config_path.parent / self.model_folder_path).resolve()
    ...
    def set_model_version(self,v):
        if v in self.supported_version:
            ...
            checkpoint_dir = self.model_folder_path / self.config["sam"]["checkpoint_dir"]
            self.model_path = checkpoint_dir / f"fish_v{v}.pth"
            if self.model:
                del self.model
